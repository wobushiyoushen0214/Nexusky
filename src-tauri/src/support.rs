  ...
  match channel {
    ...
    "template:daily-note" => {
      ...
      if !file_path.exists() {
        ...
        let content = get_config("dailyTemplate", Value::Null).as_str().map(str::to_string).unwrap_or_else(|| format!("# {date}\n\n## 今日计划\n\n- \n\n## 笔记\n\n"));
    ...
    "template:save-templates" => {
      set_config("templates", params.get("templates").cloned().unwrap_or_else(|| json!([])))?;
      Ok(Some(Value::Null))
    }
    "template:create-from" => {
      ...
      let Some(template) = templates.as_array().and_then(|items| items.iter().find(|t| t.get("id").and_then(Value::as_str) == Some(template_id))) else {
        return Ok(Some(Value::Null));
      };
      let content = template.get("content").and_then(Value::as_str).unwrap_or("# {{title}}\n\n")
        .replace("{{title}}", title)
    ...
    "ai:save-providers" => {
      set_config("aiProviders", params.get("providers").cloned().unwrap_or_else(|| json!([])))?;
      Ok(Some(Value::Null))
    }
    "ai:set-active" => {
      set_config("activeAIProvider", Value::String(as_str(&params, "providerId")?.to_string()))?;
      Ok(Some(Value::Null))
    }
    "ai:get-active-provider" => Ok(Some(get_config("activeAIProvider", Value::Null))),
    "ai:get-system-prompt" => Ok(Some(get_config("systemPrompt", Value::String(String::new())))),
    "ai:set-system-prompt" => {
      set_config("systemPrompt", Value::String(as_str(&params, "prompt")?.to_string()))?;
      Ok(Some(Value::Null))
    }
    "ai:validate" => Ok(Some(json!({ "ok": true }))),
    "ai:detect-local-config" => Ok(Some(json!({ "skipped": ["Tauri local config detection is not migrated yet"] }))),
    "ai:list-ollama-models" => Ok(Some(json!([]))),
    "cloud:get-config" => Ok(Some(get_config("cloudConfig", json!({ "supabaseUrl": "", "supabaseKey": "", "serviceRoleKey": "", "enabled": false })))),
    "cloud:save-config" => {
      set_config("cloudConfig", params.get("config").cloned().unwrap_or_else(|| json!({})))?;
      Ok(Some(Value::Null))
    }
    "cloud:get-user" => Ok(Some(Value::Null)),
    "cloud:sign-out" | "cloud:set-online" => Ok(Some(Value::Null)),
    "cloud:init" | "cloud:sign-in" | "cloud:sign-up" | "cloud:onedrive-auth" => Ok(Some(json!({ "success": false, "error": "Tauri cloud auth is not migrated yet" }))),
    "cloud:sync" | "cloud:pull-all" => Ok(Some(json!({ "total": 0, "pushed": 0, "pulled": 0, "conflicts": [], "errors": ["Tauri cloud sync is not migrated yet"] }))),
    "cloud:push-file" | "cloud:pull-file" | "cloud:push-index" | "cloud:pull-index" => Ok(Some(Value::Bool(false))),
    "cloud:sync-index" => Ok(Some(json!({ "pushed": false, "pulled": false }))),
    "cloud:get-sync-provider" => Ok(Some(get_config("syncProvider", Value::String("supabase".into())))),
    "cloud:set-sync-provider" => {
      set_config("syncProvider", Value::String(as_str(&params, "provider")?.to_string()))?;
      Ok(Some(Value::Null))
    }
    ...
    "cloud:test-connection" => Ok(Some(json!({ "ok": false, "error": "Tauri cloud provider is not migrated yet" }))),
    ...
    "cloud:set-icloud-path" => {
      set_config("icloudPath", Value::String(as_str(&params, "path")?.to_string()))?;
      Ok(Some(Value::Null))
    }
    "cloud:get-sync-exclude" => Ok(Some(get_config("syncExclude", json!([])))),
    "cloud:set-sync-exclude" => {
      set_config("syncExclude", params.get("paths").cloned().unwrap_or_else(|| json!([])))?;
      Ok(Some(Value::Null))
    }
