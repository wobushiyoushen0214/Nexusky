...
fn path_safe(file_path: &Path, vault_path: Option<&str>) -> bool {
  let Some(vault_path) = vault_path else { return true };
  let Ok(file) = file_path.canonicalize().or_else(|_| Ok::<PathBuf, std::io::Error>(file_path.to_path_buf())) else { return false };
  let Ok(vault) = Path::new(vault_path).canonicalize() else { return false };
...
pub fn list_directory(dir_path: &Path, recursive: bool) -> Result<Value, String> {
  ...
  entries_json.sort_by(|a, b| {
    let ad = a.get("isDirectory").and_then(Value::as_bool).unwrap_or(false);
    let bd = b.get("isDirectory").and_then(Value::as_bool).unwrap_or(false);
    if ad != bd {
      return bd.cmp(&ad);
    }
...
fn save_snapshot(file_path: &Path, vault_path: &Path) {
  ...
  let Ok(rel_path) = file_path.strip_prefix(vault_path) else { return };
  let history_dir = vault_path.join(".history").join(rel_path.parent().unwrap_or_else(|| Path::new("")));
  if fs::create_dir_all(&history_dir).is_err() {
    return;
  }
  let name = file_path.file_stem().and_then(|s| s.to_str()).unwrap_or("note");
  ...
  if snapshots.len() >= 50 {
    for entry in snapshots.into_iter().take(snapshots.len().saturating_sub(49)) {
...
pub fn handle(channel: &str, params: Value) -> Result<Option<Value>, String> {
  match channel {
    "file:read" => Ok(Some(Value::String(fs::read_to_string(as_str(&params, "path")?).map_err(|e| e.to_string())?))),
    "file:stat" => {
    ...
    "file:write" => {
      ...
      if let Some(vault) = vault_path.filter(|_| path.extension().and_then(|e| e.to_str()) == Some("md")) {
        save_snapshot(path, Path::new(vault));
      }
      fs::write(path, as_str(&params, "content")?).map_err(|e| e.to_string())?;
      Ok(Some(Value::Null))
    }
    "file:list" => Ok(Some(list_directory(Path::new(as_str(&params, "dirPath")?), true)?)),
    "file:list-shallow" => Ok(Some(list_directory(Path::new(as_str(&params, "dirPath")?), false)?)),
    "file:create" => {
      let path = Path::new(as_str(&params, "path")?);
      if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(|e| e.to_string())?;
      }
      fs::write(path, params.get("content").and_then(Value::as_str).unwrap_or("")).map_err(|e| e.to_string())?;
      Ok(Some(Value::Null))
    }
    "file:delete" => {
      let path = Path::new(as_str(&params, "path")?);
      ...
      if let Some(vault) = vault_path {
        ...
        let trash_path = trash_dir.join(format!("{}_{}", chrono::Utc::now().timestamp_millis(), file_name));
    ...
    "file:save-image" => {
      ...
      let base64_data = image_data.split_once(',').map(|(_, data)| data).unwrap_or(image_data);
    ...
    "file:get-history" => {
      ...
      let rel = file_path.strip_prefix(vault_path).map_err(|e| e.to_string())?;
      let history_dir = vault_path.join(".history").join(rel.parent().unwrap_or_else(|| Path::new("")));
      ...
      snapshots.sort_by(|a, b| b.get("fileName").and_then(Value::as_str).cmp(&a.get("fileName").and_then(Value::as_str)));
      snapshots.truncate(30);
      Ok(Some(Value::Array(snapshots)))
    }
    "file:restore-history" => {
      let content = fs::read_to_string(as_str(&params, "snapshotPath")?).map_err(|e| e.to_string())?;
      ...
      if let Ok(entries) = fs::read_dir(trash_dir) {
        for entry in entries.flatten() {
          ...
          if file_name.ends_with(".md") {
            let original_name = file_name.split_once('_').map(|(_, s)| s).unwrap_or(&file_name).to_string();
            items.push(json!({ "fileName": file_name, "originalName": original_name, "path": entry.path().to_string_lossy() }));
          }
        }
      }
      items.sort_by(|a, b| b.get("fileName").and_then(Value::as_str).cmp(&a.get("fileName").and_then(Value::as_str)));
      Ok(Some(Value::Array(items)))
    }
    "file:restore-trash" => {
      let trash_path = Path::new(as_str(&params, "trashPath")?);
      let file_name = trash_path.file_name().and_then(|s| s.to_str()).unwrap_or("");
      let original_name = file_name.split_once('_').map(|(_, s)| s).unwrap_or(file_name);
