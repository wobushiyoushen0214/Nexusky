...
fn ensure_default_kanban_columns(conn: &Connection) -> Result<(), String> {
  let count: i64 = conn.query_row("SELECT COUNT(*) FROM kanban_columns", [], |r| r.get(0)).map_err(|e| e.to_string())?;
  if count == 0 {
    for (i, (id, name)) in [("todo", "待办"), ("doing", "进行中"), ("done", "已完成")].iter().enumerate() {
      conn.execute("INSERT INTO kanban_columns (id, name, sort_order) VALUES (?1, ?2, ?3)", params![id, name, i as i64]).map_err(|e| e.to_string())?;
    }
  }
  Ok(())
}
...
fn collect_markdown_files(vault_path: &str) -> Vec<PathBuf> {
  WalkDir::new(vault_path)
    .into_iter()
    .filter_entry(|e| !e.file_name().to_string_lossy().starts_with('.') && e.file_name() != "node_modules")
  ...
  for line in fm_raw.lines() {
    if let Some((key, value)) = line.split_once(':') {
      ...
      if value.starts_with('[') && value.ends_with(']') {
        let items = value.trim_matches(['[', ']']).split(',').map(|s| Value::String(s.trim().trim_matches('"').to_string())).collect();
...
fn extract_title(frontmatter: &HashMap<String, Value>, content: &str, file_path: &Path) -> String {
  if let Some(title) = frontmatter.get("title").and_then(Value::as_str).filter(|s| !s.is_empty()) {
...
fn extract_links(content: &str) -> Vec<(String, String)> {
  ...
  for line in content.lines() {
    for cap in re.captures_iter(line) {
      links.push((cap[1].trim().to_string(), line.trim().chars().take(200).collect()));
    }
  }
  links
}
...
fn file_time_ms(path: &Path, created: bool) -> i64 {
  ...
  time.and_then(|t| t.duration_since(UNIX_EPOCH).ok()).map(|d| d.as_millis() as i64).unwrap_or(0)
}
...
fn index_note(conn: &mut Connection, vault_path: &str, file_path: &Path) -> Result<(), String> {
  ...
  let existing: Option<String> = conn.query_row("SELECT content_hash FROM notes WHERE file_path = ?1", params![rel], |r| r.get(0)).ok();
  ...
  tx.execute("DELETE FROM links WHERE source_note_id = ?1", params![id]).map_err(|e| e.to_string())?;
  for (target, context) in links {
    tx.execute("INSERT INTO links (source_note_id, target_title, context) VALUES (?1, ?2, ?3)", params![id, target, context]).map_err(|e| e.to_string())?;
  }

  tx.execute("DELETE FROM note_tags WHERE note_id = ?1", params![id]).map_err(|e| e.to_string())?;
  for tag in tags {
    tx.execute("INSERT OR IGNORE INTO tags (name) VALUES (?1)", params![tag]).map_err(|e| e.to_string())?;
    let tag_id: i64 = tx.query_row("SELECT id FROM tags WHERE name = ?1", params![tag], |r| r.get(0)).map_err(|e| e.to_string())?;
    tx.execute("INSERT OR IGNORE INTO note_tags (note_id, tag_id) VALUES (?1, ?2)", params![id, tag_id]).map_err(|e| e.to_string())?;
  }

  tx.execute("DELETE FROM tasks WHERE note_id = ?1", params![id]).map_err(|e| e.to_string())?;
  for (text, done) in tasks {
    tx.execute("INSERT INTO tasks (note_id, text, done) VALUES (?1, ?2, ?3)", params![id, text, if done { 1 } else { 0 }]).map_err(|e| e.to_string())?;
  }

  tx.execute("INSERT OR IGNORE INTO notes_fts_map (note_id) VALUES (?1)", params![id]).map_err(|e| e.to_string())?;
  let rowid: i64 = tx.query_row("SELECT rowid FROM notes_fts_map WHERE note_id = ?1", params![id], |r| r.get(0)).map_err(|e| e.to_string())?;
  tx.execute("DELETE FROM notes_fts WHERE rowid = ?1", params![rowid]).map_err(|e| e.to_string())?;
  tx.execute("INSERT INTO notes_fts (rowid, title, content) VALUES (?1, ?2, ?3)", params![rowid, title, content]).map_err(|e| e.to_string())?;
...
fn resolve_links(conn: &Connection, note_id: &str, title: &str) -> Result<(), String> {
  conn.execute("UPDATE links SET target_note_id = ?1 WHERE target_title = ?2 AND target_note_id IS NULL", params![note_id, title]).map_err(|e| e.to_string())?;
  conn.execute(
    r#"
...
fn remove_note_index(conn: &Connection, vault_path: &str, file_path: &Path) -> Result<(), String> {
  let rel = rel_path(vault_path, file_path)?;
  let note_id: Option<String> = conn.query_row("SELECT id FROM notes WHERE file_path = ?1", params![rel], |r| r.get(0)).ok();
  if let Some(id) = note_id {
    let rowid: Option<i64> = conn.query_row("SELECT rowid FROM notes_fts_map WHERE note_id = ?1", params![id], |r| r.get(0)).ok();
    if let Some(rowid) = rowid {
      conn.execute("DELETE FROM notes_fts WHERE rowid = ?1", params![rowid]).map_err(|e| e.to_string())?;
      conn.execute("DELETE FROM notes_fts_map WHERE note_id = ?1", params![id]).map_err(|e| e.to_string())?;
    }
  }
  conn.execute("DELETE FROM notes WHERE file_path = ?1", params![rel]).map_err(|e| e.to_string())?;
  Ok(())
}
...
pub fn handle(channel: &str, params: Value) -> Result<Option<Value>, String> {
  match channel {
    "db:index-vault" => {
      ...
      let existing: HashSet<String> = files.iter().filter_map(|p| rel_path(vault, p).ok()).collect();
      let mut stmt = conn.prepare("SELECT file_path FROM notes").map_err(|e| e.to_string())?;
      let rows = stmt.query_map([], |r| r.get::<_, String>(0)).map_err(|e| e.to_string())?;
      let stale: Vec<String> = rows.filter_map(Result::ok).filter(|p| !existing.contains(p)).collect();
      drop(stmt);
    ...
    "db:get-recent-notes" => {
      ...
      let mut stmt = conn.prepare("SELECT id, title, file_path FROM notes ORDER BY updated_at DESC LIMIT ?1").map_err(|e| e.to_string())?;
    ...
    "db:fulltext-search" => {
      let vault = as_str(&params, "vaultPath")?;
      let query = as_str(&params, "query")?.replace(['"', '\''], "").trim().to_string();
      if query.is_empty() {
        return Ok(Some(json!([])));
      }
      if params.get("regex").and_then(Value::as_bool).unwrap_or(false) {
    ...
    "db:get-backlinks" => {
      ...
      let title: Option<String> = conn.query_row("SELECT title FROM notes WHERE id = ?1", params![note_id], |r| r.get(0)).ok();
      let Some(title) = title else { return Ok(Some(json!([]))) };
    ...
    "db:get-tags" => {
      ...
      let rows = stmt.query_map([], |r| Ok(json!({ "name": r.get::<_, String>(0)?, "count": r.get::<_, i64>(1)? }))).map_err(|e| e.to_string())?;
      Ok(Some(Value::Array(rows.filter_map(Result::ok).collect())))
    }
    "db:get-notes-by-tag" => {
      let conn = connection(as_str(&params, "vaultPath")?)?;
...
fn regex_search(vault: &str, query: &str) -> Result<Value, String> {
  ...
  for file in collect_markdown_files(vault) {
    ...
    let title = content.lines().find_map(|l| l.strip_prefix("# ").map(str::trim)).unwrap_or_else(|| file.file_stem().and_then(|s| s.to_str()).unwrap_or(&rel));
...
fn get_graph(vault: &str) -> Result<Value, String> {
  let conn = connection(vault)?;
  let mut stmt = conn.prepare("SELECT id, title, file_path FROM notes").map_err(|e| e.to_string())?;
  ...
  for (id, title, file_path) in &notes {
    for key in [title.clone(), Path::new(file_path).file_stem().and_then(|s| s.to_str()).unwrap_or(title).to_string()] {
  ...
  let mut link_stmt = conn.prepare("SELECT source_note_id, target_note_id, target_title FROM links").map_err(|e| e.to_string())?;
  let links = link_stmt.query_map([], |r| Ok((r.get::<_, String>(0)?, r.get::<_, Option<String>>(1)?, r.get::<_, String>(2)?))).map_err(|e| e.to_string())?;
  let mut edge_set = HashSet::new();
  let mut edges = Vec::new();
  for link in links.filter_map(Result::ok) {
    let target = link.1.filter(|id| note_ids.contains(id)).or_else(|| title_to_id.get(&link.2).cloned());
  ...
  for (_, _, file_path) in &notes {
    if let Some(parent) = Path::new(file_path).parent().and_then(|p| p.to_str()).filter(|p| !p.is_empty()) {
      folder_map.entry(parent.to_string()).or_insert_with(|| format!("folder:{parent}"));
    }
  }
  let mut nodes = Vec::new();
  for (path, id) in &folder_map {
    let title = Path::new(path).file_name().and_then(|s| s.to_str()).unwrap_or(path);
