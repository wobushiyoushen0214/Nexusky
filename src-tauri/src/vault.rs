...
fn add_recent(config: &mut Value, vault_path: &str) {
  let existing = config.get("recentVaults").and_then(Value::as_array).cloned().unwrap_or_default();
...
pub fn handle(channel: &str, params: Value) -> Result<Option<Value>, String> {
  match channel {
    "vault:select" => {
      let Some(path) = rfd::FileDialog::new().set_title("选择笔记库目录").pick_folder() else {
    ...
    "vault:create" => {
      let Some(parent) = rfd::FileDialog::new().set_title("选择笔记库存放位置").pick_folder() else {
        return Ok(Some(Value::Null));
      };
      let name = params.get("name").and_then(Value::as_str).ok_or_else(|| "missing name".to_string())?;
    ...
    "vault:get" => {
      ...
      Ok(Some(config.get("vaultPath").cloned().unwrap_or(Value::Null)))
    }
    "vault:get-recent" => {
      let config = read_config();
      Ok(Some(config.get("recentVaults").cloned().unwrap_or_else(|| json!([]))))
    }
    "vault:clear-current" => {
      let mut config = read_config();
