...
describe('long-context schema', () => {
  ...
  it('creates long-context tables, indexes, and foreign keys for a new vault', async () => {
    ...
    expect(db.prepare('SELECT version FROM schema_version LIMIT 1').get()).toEqual({ version: 9 })
  ...
  it('migrates an existing schema 8 vault to schema 9 idempotently', async () => {
    ...
    expect(migratedDb.prepare('SELECT version FROM schema_version LIMIT 1').get()).toEqual({ version: 9 })
    ...
    expect(getDatabase(vaultPath).prepare('SELECT version FROM schema_version LIMIT 1').get()).toEqual({ version: 9 })
  })
