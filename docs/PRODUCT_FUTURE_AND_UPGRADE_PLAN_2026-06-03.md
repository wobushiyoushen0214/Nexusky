...
| 维护任务仍偏“系统视角” | 有 top 3 任务，但 skip/snooze/not relevant、批量合并、完成历史还可以更强 | 长期留存依赖维护体验不制造负担 |
| AI 来源边界还可更显性 | 已有 sources 和 retrieved notes guard，但用户未必知道本次发送了哪些片段 | 隐私敏感用户需要更强的请求前透明度 |
| 真语义检索未上线 | 当前默认是 keyword/FTS/本地词法相关检索，embedding 只保留评估方案 | 若对外称“语义搜索”会再次产生信任风险 |
| 分发信任仍未完全闭环 | v0.8.2 为 unsigned packaging 路径，长期商业化仍需要签名、公证、更新可信链 | 影响非开发者用户安装和升级信任 |
...
| P0 | 今日 3 件事更智能 | 按影响范围、修复可逆性、用户反馈和预计耗时排序 | top 3 点击率和完成率可统计 |
...
| P1 | 对话级成本预算 | 每次发送前估算 tokens/cost，支持月预算提醒 | 用量超阈值前提示，不能静默超支 |
...
| 优先级 | 升级项 | 说明 | 验收标准 |
| --- | --- | --- | --- |
| P0 | Graph 任务化 | 图谱默认回答“哪里断裂、哪里孤立、哪里跨域连接” | Graph 空状态和默认 filter 与 Maintenance 联动 |
...
| P1 | Properties 变成轻量数据库视图 | 支持按 frontmatter property 过滤、排序、批量补字段 | 保持 Markdown/frontmatter，不引入重对象系统 |
...
| P0 | 同步灾难恢复演练 | 人为制造删除、冲突、远端旧版本、本地改动覆盖 | 文档和测试覆盖恢复路径 |
...
- 每个样例包含模板和 `Maintenance/Workflow Rules.md`。
...
| P1 | 模板和维护规则联动 | 根据 workflow rules 自动影响 maintenance required properties | 样例规则不是静态说明，而能驱动队列 |
...
目标：把 v0.8.2 已经落地的核心闭环打磨到可被真实用户放心试用。
...
| P1-3 | Graph 维护任务化 | 图谱应产生行动，而不是只做浏览 |
| P1-4 | AI cost budget | BYO Provider 用户需要成本透明 |
| P1-5 | Workflow rules 驱动维护队列 | 样例、模板和维护建议需要闭环 |
| P1-6 | 同步灾难恢复演练 | 商业化备份前必须证明恢复可靠 |

### P2：可以等待

| ID | 事项 | 原因 |
| --- | --- | --- |
| P2-1 | 真 embedding 检索 | 需要隐私、成本、质量评估后再进入默认路径 |
| P2-2 | 插件市场 | 平台化会分散当前产品主线 |
| P2-3 | 团队协作 | 权限和实时协作会显著改变架构复杂度 |
| P2-4 | 移动端完整编辑 | 当前桌面闭环还未完全商业化 |
| P2-5 | 对象化数据库系统 | 会偏离 Markdown source of truth |
...
- Sync disaster recovery fixture。
