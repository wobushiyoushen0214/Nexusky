...
| P2-2 | 插件市场（本地内置元数据已完成） | 平台化会分散当前产品主线 |
...
| 2026-06-04 | P2-2 插件市场（本地内置元数据） | 完成 | 精选插件入口明确收束为 `bundled_local` 本地内置包；`PluginMarketplaceItem` 新增 `source`、`permissions`、`riskLevel` 和 `installNote`，IPC 会从声明式 commands/panels/editorExtensions 推导 `ai_prompt`、`read_only_panel`、`editor_extension_declaration` 权限；设置页 marketplace 卡片显示来源、风险和权限 chips；插件文档补充“不联网下载、不执行远程代码”的市场边界。 | `pnpm vitest run tests/plugin-api.test.ts`；`pnpm typecheck` |
