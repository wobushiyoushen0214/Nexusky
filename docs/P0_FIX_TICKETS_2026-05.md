...
> 分发侧补齐（需证书/发布流水线 secrets 配合）：macOS 发布流水线已要求 Developer ID 签名、公证与 zip 更新产物；Windows 发布流水线已要求 Authenticode 签名并开启更新签名校验。本次提交已修复应用内 “XSS → 任意读 → 明文 key → 无 CSP/遥测外发” 安全链。
...
| Windows 签名发布 | Windows 构建移除 `sign:false`，强制 Authenticode 签名并开启更新签名校验；缺少 `WIN_CSC_*` secrets 时 CI 失败 | ✅ 已修复（本提交） |
