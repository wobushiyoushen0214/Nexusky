...
| P1 | 知识图谱大图不再用 DOM 渲染所有节点+边：超过阈值自动切到视口 Canvas raster renderer（✅ 本提交） | `GraphView.tsx`、`graph-types.ts` |
...
| P2 | Toast 无 `aria-live`；Settings 模态无 `role=dialog`/焦点陷阱/无 Esc 关闭（✅ 本提交） | `Toast.tsx`、`Settings.tsx` |
