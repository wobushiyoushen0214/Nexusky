...
| Agent run controls | `components/agent/AgentRunPanel.tsx` | `Input`, `Textarea`, `Button`, `Checkbox`, `Card`, `Badge`, `Empty` | Done for goal/description fields, plan edit fields, action buttons, dry-run checkbox, history rows, plan/execute step cards, plan count, plan/execute step labels, and empty states. Execution workflow layout remains custom. |
