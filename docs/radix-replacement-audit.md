...
- `VaultOverview` header refresh action now uses shared `Button`; summary metric cards, the vitality card, and overview chart cards now use shared `Card` surfaces while preserving the existing overview glass styling and chart behavior.
...
| Overview cards | `Card`, `Badge` | Shared `Card` is now used for summary metric cards, the vitality card, and chart card surfaces, with existing glass CSS and chart interactions preserved. Badge remains optional for future value/status chips. |
