...
- Publish scope dialog now uses `Dialog`, `ScrollArea`, `Button`, `ToggleGroup`, `Input`, `Empty`, `Spinner`, and the shared `ConfirmModal`.
...
- `BasesView` toolbar search, inline property edit fields, toolbar actions, tag/sort filters, column picker actions, column checkboxes, tag lens controls, note title actions, inline edit targets, and main empty/loading state now use shared `Input`, `Button`, `Select`, `Checkbox`, `Empty`, and `Spinner`.
...
| Bases controls | `components/bases/BasesView.tsx` | `Input`, `Button`, `Select`, `Checkbox`, `Empty`, `Spinner` | Done for toolbar search, inline property edit fields, toolbar actions, tag/sort filters, column picker actions, column checkboxes, tag lens controls, note title actions, inline double-click edit targets, and main empty/loading state. |
...
| Publish scope inputs | `Input`, `Empty`, `Spinner` | Done for folder, tag, property key, and property value filters while preserving native `datalist` suggestions and publish preview/publish flow. Preview panel loading/empty states now use shared feedback primitives. |
