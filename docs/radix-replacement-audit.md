...
| Long context debug panel | `components/observability/LongContextDebugPanel.tsx` | `Card`, `CardHeader`, `CardTitle`, `CardDescription`, `CardContent`, `Tabs`, `Slider`, `Button`, `Badge`, `Tooltip`, `Empty`, `Spinner` | Done for inspector/metrics/tuning section cards/titles, pack item card composition, metric tiles, pack tier switching, tuning sliders, tuning actions, tier/token and metrics count feedback, metric trend hints, empty states, and loading states. |
