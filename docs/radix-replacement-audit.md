...
- `MemoryTimelinePanel` now uses those wrappers, including shared `Badge` for tier/count labels, shared `Tooltip` for source hints, shared `Empty`/`Spinner` for its loading state, and shared `Empty` for its empty state.
...
- Publish scope dialog now uses `Dialog`, `ScrollArea`, `Button`, `ToggleGroup`, `Input`, `Badge`, `Tooltip`, `Empty`, `Spinner`, `Card`, and the shared `ConfirmModal`, including the preview and issue list scroll regions plus the published target panel.
...
- `GraphMaintenanceNudge` now uses shared `Button` and `Tooltip` for focus pills, clear, collapse, and expand controls, plus shared `Badge` for maintenance count chips while preserving graph canvas behavior.
- `AgentRunPanel` goal/description and plan edit fields now use shared `Textarea`/`Input`, action controls and history row open actions use shared `Button`, dry-run option uses shared `Checkbox`, history rows and plan/execute step rows use shared `Card`, plan count, status, and plan/execute step labels use shared `Badge`, and empty states use shared `Empty`; the existing agent workflow layout remains custom.
...
- `TagsPanel` now uses shared `Card` composition with `CardHeader`/`CardTitle`/`CardContent` for the current-file tag section, plus shared `Input`, `Button`, `Badge`, `ScrollArea`, `Tooltip`, and `Empty`; current tags, tag counts, tag chip/list styles were moved into token-based local CSS.
- `VaultHealthScreen` action controls now use shared `Button` for skip and next-step actions, status/kicker chips use shared `Badge`, state/hero/driver/metric/next-step panels use shared `Card`, score completion uses shared `Progress`, and the no-issue driver state uses shared `Empty`, with styling aligned to shared tokens.
- `ErrorBoundary` fallback now uses shared `Card` with `CardHeader`/`CardTitle`/`CardContent`/`CardFooter` composition, shared `Alert` for the error callout, and shared `Button` for retry, with the fallback layout kept in token-based local CSS.
- `ChatSourceRow` now uses shared `Card`/`CardHeader`/`CardTitle`/`CardContent` composition for citation lookup explanation/result sections, shared `Button`, `Popover`, `Tooltip`, `ScrollArea`, `Spinner`, and compact `Empty` feedback for citation lookup controls instead of custom absolute-positioned popovers, document listeners, native title hints, and ad hoc loading/empty text.
- `LongContextDebugPanel` now uses shared `Card`/`CardTitle` composition for inspector/metrics/tuning sections, shared `Card` composition with `CardHeader`/`CardTitle`/`CardDescription`/`CardContent` for pack items and metric tiles; shared `Tabs` for pack tier switching; shared `Slider` for tuning controls; shared `Button` for tuning actions; shared `Badge` for tier/token and metrics count feedback; shared `Tooltip` for metric trend hints; and shared `Empty`/`Spinner` for empty and loading states, with local observability styles mapped to shadcn semantic tokens.
- `ProactivePreferences` now uses shared `Card`/`CardTitle` composition for preference groups, shared `Input`, `Slider`, `Checkbox`, `Button`, `Spinner`, and `Empty` controls, with the settings form/loading styling moved from inline objects into token-based proactive CSS.
...
- `CloudSyncSettings` info/config/status panels now use shared `Card` composition with `CardHeader`/`CardTitle`/`CardContent` where applicable, the provider select now groups items with `SelectGroup`, and `PluginsSettings` plugin rows now use shared `Card` composition with `CardHeader`/`CardContent`; CloudSync status values and plugin version labels now use shared `Badge`.
...
- `ChatMessages` now uses shared `Empty` for the prompt empty state, shared `Button` for empty-state prompt hints, and shared `Spinner` for streaming/tool status, with hover/loading styling moved into local token-based CSS.
...
- `VaultOverview` header refresh action now uses shared `Button`; summary metric cards now use shared `CardContent`/`CardTitle`/`CardDescription` composition, the vitality card now uses shared `CardHeader`/`CardTitle`/`CardDescription`/`CardContent`/`CardFooter`, and overview chart cards use shared `Card` surfaces; shared `OverviewCard` composes chart panels with `CardHeader`/`CardTitle`/`CardDescription`/`CardContent`; the vitality score chip uses shared `Badge`; vitality scanning feedback uses shared `Spinner`; composition empty feedback now uses shared `Empty`, while preserving the existing overview glass styling and chart behavior.
...
- `BacklinksPanel` now uses shared `Button` for the collapse header, outgoing row jumps, target open/create actions, and unlinked-mention conversion actions; shared `Card` for link row surfaces; and shared `Badge` for counts/line labels while preserving its compact link overview layout.
...
- `MermaidRenderer` syntax error feedback now uses shared `Alert` while preserving the markdown preview renderer and Mermaid SVG rendering path.
- `Sidebar` shell controls now use shared `Input` for create/filter fields, shared `DropdownMenu` for the vault switcher, shared `Button` for create actions, sort/expand controls, favorites, and file-error recovery, shared `Alert` for file/index error callouts, and shared `Tooltip` for the vault switcher and shell action hints while preserving the existing file tree and virtual scrolling behavior.
- `FileTree` and `VirtualFileTree` rename/create inputs now use shared `Input`, row/more-action controls use shared `Button`, more-action hints use shared `Tooltip`, and empty vault states use shared `Empty`, while preserving tree drag/drop, expand/collapse, context menu coordinates, row height, selection, virtual scrolling, and active-file behavior.
- `ActivityBar` rail and settings buttons remain on their original native `<button>` rendering; only its coordinate context menu bridge uses the shared compatibility layer. Do not migrate the rail buttons through shared controls because their active border, shadow, hover, and liquid indicator styling must remain unchanged.
...
| Memory timeline | `components/memory/MemoryTimelinePanel.tsx` | Already migrated partly | Already uses `Dialog`, `ScrollArea`, `Button`, `Badge`, `Tooltip`, `Spinner`, `Empty` for modal shell, timeline actions, tier/count labels, source hints, loading, and empty states. Loading state is now composed with shared `Empty`/`Spinner`. |
...
| Related context tabs | `components/long-context/RelatedContextPanel.tsx` | `Tabs`, `ScrollArea`, `Card`, `CardHeader`, `CardTitle`, `CardDescription`, `CardContent`, `Button`, `Badge`, `Skeleton`, `Alert`, `Empty`, `Tooltip` | Done for pack summary/tier tabs, pack item card composition, count badges, controls, compact pack empty state, panel navigation/refresh tooltips, loading placeholders, and error alert. Native list scrolling remains to preserve side/page layout sizing. |
| Long context badges | `components/long-context/LongContextBadge.tsx` | `Badge` | Done. Uses local Badge with existing class styling. |
| Long context debug panel | `components/observability/LongContextDebugPanel.tsx` | `Card`, `CardHeader`, `CardTitle`, `CardDescription`, `CardContent`, `Tabs`, `Slider`, `Button`, `Badge`, `Tooltip`, `Empty`, `Spinner` | Done for inspector/metrics/tuning section cards/titles, pack item and metric tile card composition, pack tier switching, tuning sliders, tuning actions, tier/token and metrics count feedback, metric trend hints, empty states, and loading states. |
| Related context card actions | `components/long-context/RelatedContextCard.tsx` | `Card`, `CardHeader`, `CardTitle`, `CardContent`, `CardFooter`, `Button` with `variant="ghost"` and `size="icon"`, `Tooltip` | Done for card surfaces, header/title/body/footer composition, title/open, and icon feedback actions. Icon feedback actions now use shared tooltip content with aria-labels retained. |
...
| Graph maintenance nudge | `components/graph/GraphMaintenanceNudge.tsx` | `Button`, `Tooltip`, `Badge` | Done for focus, clear, collapse, and expand controls, action hints, and maintenance count chips. Graph canvas interaction layers remain custom. |
| Agent run controls | `components/agent/AgentRunPanel.tsx` | `Input`, `Textarea`, `Button`, `Checkbox`, `Card`, `Badge`, `Empty` | Done for goal/description fields, plan edit fields, action buttons, history row open actions, dry-run checkbox, history rows, plan/execute step cards, plan count, status chip, plan/execute step labels, and empty states. Execution workflow layout remains custom. |
...
| Settings page controls | `components/settings/pages/*` | `Switch`, `Checkbox`, `RadioGroup`, `Select`, `Input`, `Button`, `Card`, `CardHeader`, `CardTitle`, `CardContent`, `Badge`, `Spinner`, `Empty`, `Alert` | Done for core switch/select/button controls, `LongContextSettings` numeric inputs, the Appearance custom accent text field and tokenized option surfaces, Proactive category row cards and tokenized option surfaces, the Keys key-capture field and keybinding row Card composition, shared settings page loading states, plugin/keybinding empty states, CloudSync Card composition/status badges/select grouping, Plugins Card composition, and CloudSync sync errors. Remaining native inputs are specialized native color picker only. |
| Proactive preferences | `components/proactive/ProactivePreferences.tsx` | `Card`, `CardTitle`, `Input`, `Slider`, `Checkbox`, `Button`, `Spinner`, `Empty` | Done for preference group cards/titles, silent-hour fields, threshold sliders, enable/per-kind options, loading state, and debug/reset actions while preserving save-on-change behavior. |
...
| Overview cards | `Card`, `CardHeader`, `CardTitle`, `CardDescription`, `CardContent`, `CardFooter`, `Badge`, `Spinner`, `Empty` | Shared `Card` composition is now used for summary metric cards, the vitality card, and `OverviewCard` chart panels; the vitality score chip uses shared `Badge`; vitality scanning feedback uses shared `Spinner`; composition empty feedback uses shared `Empty`, with existing glass CSS and chart interactions preserved. |
| History panel | `Card`, `CardHeader`, `CardTitle`, `CardFooter`, `Button`, `Badge`, `ScrollArea`, `Empty` | Done. Snapshot rows now use shared Card composition with semantic `time` titles and shared footer actions; kept the lightweight side-panel behavior and moved inline/count/empty styles into local token-based CSS. |
| Tags panel | `Card`, `CardHeader`, `CardTitle`, `CardContent`, `Input`, `Button`, `Badge`, `ScrollArea`, `Tooltip`, `Empty` | Done. Current-file tag section composition, current tag input, tags, tag count badges, tag action tooltips, tag cloud, empty state, and filtered note list now use shared components with local token-based CSS. |
| Error boundary | `Card`, `CardHeader`, `CardTitle`, `CardContent`, `CardFooter`, `Alert`, `Button` | Done. Fallback surface now uses shared Card composition, error callout uses shared Alert, retry action uses shared Button, and fallback layout remains token-based local CSS. |
| Citation source row | `Card`, `CardHeader`, `CardTitle`, `CardContent`, `Button`, `Popover`, `Tooltip`, `ScrollArea`, `Spinner`, `Empty` | Done. Citation lookup explanation/results now use shared Card composition inside a shared Popover with token-based local CSS, shared action hints, and shared Empty/Spinner loading feedback. |
| Welcome screen | `Card`, `Input`, `Button`, `Tooltip` | Done for hero/start/sample surfaces, vault-name creation input, demo, open/create, recent vault, sample vault actions, and recent vault path hints. Existing entry-page layout and visual classes remain in place. |
...
| Demo flow | `Dialog`, `Card`, `Button`, `Badge`, `Spinner`, `Progress` | Done for modal shell, close/sample/fix/done actions, sample/issue/comparison/fixes surfaces, scenario/problem count labels, scan loading, and pre-fix health score progress. |
...
| Chat message prompt hints | `Empty`, `Button`, `Spinner` | Done for the prompt empty state, empty-state prompt hint actions, and streaming/tool status feedback. Streaming message layout remains custom. |
| Chat panel actions and batch inputs | `Button`, `ToggleGroup`, `Input`, `Badge`, `Tooltip` | Done for outbound preview actions, chat header actions/hints, session list actions, batch-plan controls, batch-plan directory/topic/count inputs, batch-plan remove action hints, folder picker actions, folder-picker new-directory input, edit preview actions, edit-target action hints, mention result rows, attached note/document chips, attachment removal controls, and composer icon actions/hints. ChatPanel no longer uses raw `<button>` elements. Composer textarea and hidden file input remain native by design. |
...
| Backlinks panel actions | `Button`, `Card`, `Badge` | Done for collapse, outgoing row jumps, target open/create, batch convert, item jump, single mention conversion actions, row surfaces, counts, and line labels. The list layout remains custom. |
...
| Mermaid preview feedback | `Alert` | Done for Mermaid syntax error feedback. Mermaid rendering, SVG sanitization, and editor preview layout remain custom. |
| Publish scope inputs | `Input`, `Badge`, `Tooltip`, `ScrollArea`, `Empty`, `Spinner`, `Card` | Done for folder, tag, property key, property value filters, current target path hints, published target card/pills, and preview/issue list scrolling while preserving native `datalist` suggestions and publish preview/publish flow. Preview panel loading/empty states, child-list empty states, and status badge now use shared feedback primitives. |
| Sidebar shell controls | `Input`, `DropdownMenu`, `Button`, `Tooltip`, `Alert` | Done for create/filter fields, vault switcher dropdown/hint, create file/folder actions, sort/collapse/expand controls, favorites, file-error recovery actions, file/index error callouts, and shell action hints. File tree row rendering and virtual scrolling remain custom. |
| File tree rows | `Input`, `Button`, `Tooltip`, `Empty` | Done for `FileTree` and `VirtualFileTree` rename/create inputs, row and more-action controls, more-action hints, and empty vault states. Drag/drop, context menu coordinates, expand/collapse, active row state, multi-select, and virtual scroll math remain custom. |
| ActivityBar controls | Do not migrate rail buttons | Native rail/settings button rendering is intentionally preserved. Only the right-click menu bridge can use shared menu primitives. Overview remains the pinned first item and the default landing view. |
