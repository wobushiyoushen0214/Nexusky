...
| Overview cards | `Card`, `CardHeader`, `CardTitle`, `CardDescription`, `CardContent`, `Badge`, `Empty` | Shared `Card` composition is now used for summary metric cards and `OverviewCard` chart panels; the vitality card and chart surfaces use shared Card surfaces; the vitality score chip uses shared `Badge`; composition empty feedback uses shared `Empty`, with existing glass CSS and chart interactions preserved. |
