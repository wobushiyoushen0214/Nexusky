...
| Related context tabs | `components/long-context/RelatedContextPanel.tsx` | `Tabs`, `ScrollArea`, `Button`, `Badge`, `Skeleton`, `Alert`, `Empty` | Done for pack summary/tier tabs, controls, compact pack empty state, loading placeholders, and error alert. Native list scrolling remains to preserve side/page layout sizing. |
