...
- `VaultOverview` header refresh action now uses shared `Button`; summary metric cards, the vitality card, and overview chart cards now use shared `Card` surfaces; shared `OverviewCard` now composes chart panels with `CardHeader`/`CardTitle`/`CardDescription`/`CardContent`; the vitality score chip uses shared `Badge`; composition empty feedback now uses shared `Empty`, while preserving the existing overview glass styling and chart behavior.
