...
- `LongContextDebugPanel` now uses shared `Card`/`CardTitle` composition for inspector/metrics/tuning sections, shared `Card` for pack items and metric tiles; shared `Tabs` for pack tier switching; shared `Slider` for tuning controls; shared `Button` for tuning actions; shared `Badge` for tier/token and metrics count feedback; shared `Tooltip` for metric trend hints; and shared `Empty`/`Spinner` for empty and loading states, with local observability styles mapped to shadcn semantic tokens.
...
| Long context debug panel | `components/observability/LongContextDebugPanel.tsx` | `Card`, `CardTitle`, `Tabs`, `Slider`, `Button`, `Badge`, `Tooltip`, `Empty`, `Spinner` | Done for inspector/metrics/tuning section cards/titles, pack item cards, metric tiles, pack tier switching, tuning sliders, tuning actions, tier/token and metrics count feedback, metric trend hints, empty states, and loading states. |
