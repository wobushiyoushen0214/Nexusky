...
- `LongContextDebugPanel` now uses shared `Tabs` for pack tier switching, shared `Slider` for tuning controls, shared `Button` for tuning actions, shared `Badge` for tier/token count feedback, and shared `Empty`/`Spinner` for empty and loading states, with local observability styles mapped to shadcn semantic tokens.
...
| Long context debug panel | `components/observability/LongContextDebugPanel.tsx` | `Tabs`, `Slider`, `Button`, `Badge`, `Empty`, `Spinner` | Done for pack tier switching, tuning sliders, tuning actions, tier/token count feedback, empty states, and loading states. |
