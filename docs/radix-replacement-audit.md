...
- `VaultOverview` header refresh action now uses shared `Button`; summary metric cards, the vitality card, and overview chart cards now use shared `Card` surfaces; composition empty feedback now uses shared `Empty`, while preserving the existing overview glass styling and chart behavior.
...
| Overview cards | `Card`, `Badge`, `Empty` | Shared `Card` is now used for summary metric cards, the vitality card, and chart card surfaces; composition empty feedback uses shared `Empty`, with existing glass CSS and chart interactions preserved. Badge remains optional for future value/status chips. |
