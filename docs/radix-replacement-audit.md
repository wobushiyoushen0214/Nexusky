...
- `ErrorBoundary` fallback now uses shared `Card` with `CardContent`/`CardFooter` composition, shared `Alert` for the error callout, and shared `Button` for retry, with the fallback layout kept in token-based local CSS.
...
| Error boundary | `Card`, `CardContent`, `CardFooter`, `Alert`, `Button` | Done. Fallback surface now uses shared Card composition, error callout uses shared Alert, retry action uses shared Button, and fallback layout remains token-based local CSS. |
