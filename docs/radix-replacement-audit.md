...
- `RelatedContextPanel` context-pack summary and tiers now use shared `Button`/`Tabs`; panel counts, pack summary counts, tier counts, and relation labels use shared `Badge`; pack tier empty state uses shared `Empty`; related context card title/actions use shared `Button`, and icon feedback actions use shared `Tooltip`; loading placeholders use shared `Skeleton`; error state uses shared `Alert`.
...
- `TagsPanel` now uses shared `Input`, `Button`, `Badge`, `ScrollArea`, and `Empty`; current tags, tag counts, tag chip/list styles were moved into token-based local CSS.
...
- `NotificationCenter` now uses shared `Button` for bell, bulk actions, close, open, snooze, and dismiss controls; bell count, drawer count, and suggestion kind labels use shared `Badge`; drawer empty state uses shared `Empty`; notification button styling now reads from shadcn semantic tokens.
...
| --- | --- | --- | --- |
| Notification center | `components/proactive/NotificationCenter.tsx` | `Sheet`, `DropdownMenu`, `Button`, `Badge`, `Empty` | Done for drawer shell, snooze menu, bell, bulk actions, close, open, snooze, dismiss controls, drawer/bell/kind badges, and empty state. |
| Related context tabs | `components/long-context/RelatedContextPanel.tsx` | `Tabs`, `ScrollArea`, `Button`, `Badge`, `Skeleton`, `Alert`, `Empty` | Done for pack summary/tier tabs, count badges, controls, compact pack empty state, loading placeholders, and error alert. Native list scrolling remains to preserve side/page layout sizing. |
...
| Tags panel | `Input`, `Button`, `Badge`, `ScrollArea`, `Empty` | Done. Current tag input, tags, tag count badges, tag cloud, empty state, and filtered note list now use shared components with local token-based CSS. |
