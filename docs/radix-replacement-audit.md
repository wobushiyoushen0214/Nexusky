...
- `CloudSyncSettings` info/config/status panels now use shared `Card` composition with `CardHeader`/`CardContent` where applicable, the provider select now groups items with `SelectGroup`, and `PluginsSettings` plugin rows use shared `Card`; CloudSync status values and plugin version labels now use shared `Badge`.
