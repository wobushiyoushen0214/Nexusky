...
- `ChatPanel` outbound preview actions, chat header actions, and session list actions now use shared `Button` while preserving the compact glass chat layout and existing hover colors.
...
| Chat panel actions | `Button` | Done for outbound preview actions, chat header actions, and session list actions. Input composer, attachment chips, edit preview, and batch-plan controls remain split into later low-risk batches. |
