...
| A5 | 长期上下文用户化命名 | 完成 | long-context / observability 相关 UI | 将 debug 术语包装为 Memory Ledger、Context Pack、Why this |
| A6 | Agent 入口继续后置 | 完成 | Command / Chat / Maintenance 相关入口 | Agent 不作为普通用户首要入口，只在任务执行时出现 |
...
| 2026-05-31 | 知识维护更新时间格式可读化 | 完成 | 维护队列更新时间从 ISO 字符串改为本地可读格式 `YYYY-MM-DD HH:mm`，前端兜底会把旧的 `Updated: 2026-...T...Z` 或 `更新于：2026-...T...Z` 转换为同一格式；补充测试确保中文详情不再包含 `T` / `Z` |
