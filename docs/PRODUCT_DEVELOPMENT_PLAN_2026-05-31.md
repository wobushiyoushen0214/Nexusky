...
后续风险：当前完成的是维护扫描缓存键、同步结果缓存、扫描状态协议、后端分组扫描入口和 watcher/indexer 文件级 query-cache 失效；A2.4 的真实 partial results、UI 分批展示与维护项级 diff 仍未完成。不同 query/type 仍会生成独立缓存项，后续需要把底层扫描结果与前端过滤拆开，减少重复构建。
