...
后续风险：A2 性能基础已覆盖缓存键、同步结果缓存、扫描状态协议、后端分组扫描、watcher/indexer 文件级 query-cache 失效、UI partial results 和性能日志；后续仍需做更细的维护项级 diff / 后台 worker 化，减少不同 query/type 的重复构建。
