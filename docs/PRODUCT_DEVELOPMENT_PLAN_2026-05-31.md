...
后续风险：当前完成的是维护扫描缓存键、同步结果缓存、扫描状态协议、后端分组扫描入口、watcher/indexer 文件级 query-cache 失效和 UI partial results；后续仍需补 A2.5 性能日志，以及更细的维护项级 diff / 后台 worker 化，减少不同 query/type 的重复构建。
