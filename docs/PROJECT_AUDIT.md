...
| 渲染层组件 | 80+ 个，最大 `ChatPanel.tsx` 2267 行、`CanvasView.tsx` 1858 行、`GraphView.tsx` 1032 行、`Editor.tsx` 944 行 |
...
6. **检索现状更新（2026-05-30）**：旧版 `services/embedding.ts` 已更名为 `services/search-index.ts`。当前检索是本地 TF-IDF/词法相关检索 + 可选 LLM 重排，不再包装成“向量/embedding”能力。
...
6. **长上下文关系候选用 token Jaccard，不是 embedding**：`services/long-context/relation-candidates.ts:381-460` 用 `tokenSetSimilarity`（关键词集合相似度），阈值 0.12/0.16。当前已把相关信号命名为 `lexical_chunk`，跨笔记关系仍是浅层匹配 + AI confidence；后续若要向量化，需要另接真实 embedding。
...
9. **Activity Bar 入口去重仍需收尾**：Kanban 独立 surface 已在 2026-05-31 删除（`KanbanPanel.tsx` 与 `mainView === 'kanban'` 路由不再存在），但 `rightPanel === 'agent'` 仍作为高级执行历史 surface 保留。下次应继续评估 Agent 面板是否只保留为 Chat/维护队列内的执行记录。
...
| 本地相关检索 | **Beta** | search-index.ts 使用 chunk + TF-IDF/词法匹配 + 可选 LLM 重排；不是向量 embedding |
...
| 自己做 embedding 模型 | 当前未接真实 embedding；若未来要做向量检索，应在 provider/本地模型之间明确选择 |
...
- [x] WelcomeScreen → Vault Health Scan *(2026-05-28: 新增 services/vault-health.ts + IPC vault:health-scan / vault:health-shown / vault:health-mark-shown；打开 vault 后若距上次展示 > 7 天则自动跳到 VaultHealthScreen，展示笔记/链接/孤岛/未解析/重复标题/无记忆/陈旧 7 个指标 + 3 个下一步按钮（问 AI / 修未解析链接 / 浏览图谱）)*
- [x] AI Provider 配置后加测试问题 *(2026-05-28: 新增 ai:probe-question IPC，超时 30s 用当前 config 跑一次非流式问答；Settings 提供商编辑面板新增「测试问题」按钮，回答 + 延迟在表单内联展示，便于在保存前确认能拿到首个回答)*
...
- [x] Knowledge Maintenance 文案语言贯通 *(2026-05-31: 维护队列 action/reason/detail、apply-fix 结果、预览摘要、路径错误和 `plan_knowledge_maintenance` 工具结果已接入当前 UI 语言；主进程服务直调保留英文默认值，IPC 和工具调用按应用语言解析)*
