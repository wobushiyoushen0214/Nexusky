...
| 属性数据库 | Bases-like 主视图按 frontmatter 属性浏览、搜索、标签筛选和排序笔记，支持自定义显示列、双击单元格直接编辑属性，保留数字/布尔属性类型，并内置使用说明 |
...
| 自由画布 | Canvas-like 主视图将笔记作为可拖拽卡片整理，支持向上/下/左/右继续展开的虚拟画布，位置按 vault 持久化，自动叠加 wikilink 连接线，支持新建卡片、缩放、适应视图和内置使用说明 |
...
|------|------|
| 深色主题 | OKLCH 色彩系统 |
| 主题切换 | 亮/暗模式 + Obsidian / Nord / Solarized / 高对比等多主题色板，支持自定义强调色和 vault 级 CSS 片段 |
