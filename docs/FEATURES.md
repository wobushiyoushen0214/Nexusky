...
| 属性数据库 | Bases-like 主视图按 frontmatter 与 Obsidian Dataview `key:: value` 内联属性浏览、搜索、标签筛选和排序笔记，支持普通列表和自定义任务状态中的 Dataview 字段、自定义显示列、双击单元格直接编辑属性，保留数字/布尔属性类型；编辑已有 Dataview 内联字段时会原地写回，并内置使用说明 |
...
| 嵌入式笔记 | `![[笔记名]]` 内联显示引用笔记正文，兼容 `![[笔记#标题]]` 和 `![[笔记#^块]]` 局部嵌入，发布站点也会展开嵌入内容并纳入站内搜索 |
| Wikilink 预览 | hover `[[链接]]` 显示目标笔记正文摘要，`[[笔记#标题]]` / `[[笔记#^块]]` 只预览对应局部内容 |
...
| AI Agent 连接机会 | Agent 可发现尚未互链但共享标签、属性或记忆概念的笔记对，输出补 wikilink 的具体理由、建议补链方向和精确 wikilink |
| AI Agent 知识桥梁 | Agent 可识别连接多个文件夹或标签簇的桥梁笔记，提示哪些综合节点最值得优先维护 |
| AI Agent 维护队列 | Agent 可把断链、过期任务、今日到期/即将到期任务、高优先级任务、已排程/已开始任务、阻塞/等待任务、重复任务、孤岛、空笔记、重复标题/别名、开放任务、未链接引用、超长笔记、必填属性缺失、AI 记忆缺失/过期和知识桥梁综合成下一步维护队列，按优先级给出具体整理动作，并可按维护类型和即将到期窗口筛选，兼容 Dataview 与 Tasks 插件常见 due/priority/scheduled/start/status/repeat 标记 |
...
| 功能 | 说明 |
|------|------|
| 快速切换 | Ctrl+O 模糊搜索笔记，支持按标题、alias、相对路径和粘贴的 Obsidian wikilink / 嵌入语法匹配，显示文件目录路径 |
...
| 笔记索引 | 标题、链接关系、标签自动提取 |
| 索引语法边界 | 链接、标签、任务、未链接提及和 Dataview 属性索引会跳过 Obsidian 注释、代码块、行内代码和 Markdown 图片 alt 文本 |
| 标签系统 | `#tag` 语法自动解析，支持 Obsidian 嵌套标签，按标签筛选，并避免把 URL fragment 误识别为标签 |
| 增量更新 | 保存时自动更新索引 |
...
| Frontmatter 兼容 | 属性编辑统一通过 frontmatter 工具写回，兼容 CRLF 换行、Obsidian `cssclass` 旧字段、空格分隔标签/ CSS 类和标签栏/标签面板同步更新 |
...
| AI Agent 精读工具 | 支持读取整篇笔记、当前笔记、指定行范围、指定 heading 章节和 Markdown 块引用，可直接识别 Obsidian wikilink / 嵌入语法，读取结果包含标题、路径、行号等元数据 |
