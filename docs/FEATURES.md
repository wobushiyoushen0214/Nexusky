...
| 阅读收件箱 | 主视图集中浏览 Notion、Readwise 和 Pocket 导入内容，内置三类导入入口，支持来源筛选、待读筛选、全文检索、按更新/标题/来源排序、原文打开、阅读笔记追加、一键生成带高亮摘录和来源路径元数据的阅读摘要、归档隐藏、单条状态写回、当前筛选结果批量标记已读和批量归档 |
...
| 标签面板 | 右面板显示所有标签，点击筛选笔记 |
| 日历面板 | 右面板日历视图，点击日期打开每日笔记 |
| 阅读收件箱 | 主视图聚合第三方阅读材料，内置 Notion/Readwise/Pocket 导入入口，按来源、状态和关键词筛选，并可按更新、标题或来源排序；可打开原文或原笔记、追加阅读笔记，把当前结果生成带高亮/笔记摘录和来源路径元数据的阅读摘要笔记，归档隐藏条目，并可单条或批量写回阅读状态、批量归档当前结果 |
