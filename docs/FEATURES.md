...
| 属性数据库 | Bases-like 主视图按 frontmatter 属性浏览、搜索、标签筛选和排序笔记，支持双击单元格直接编辑属性 |
...
| 知识库发布 | 命令面板可将整个 vault 导出为静态 HTML 站点，保留目录索引和 wikilink 跳转 |
...
| 自由画布 | Canvas-like 主视图将笔记作为可拖拽卡片整理，位置按 vault 持久化，自动叠加 wikilink 连接线，双击打开笔记 |
...
|------|------|
| 深色主题 | OKLCH 色彩系统 |
| 主题切换 | 亮/暗模式 + Obsidian / Nord / Solarized / 高对比等多主题色板 |
